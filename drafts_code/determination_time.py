...
class DurationCalculator(QDialog):
    ...
    def calculate_experiment_duration(self):
        try:
            ...
            # Load data
            heartrate_data = pd.read_csv(heartrate_file) if heartrate_file.endswith('.csv') else pd.read_excel(
                heartrate_file)
            tobii_data = pd.read_csv(tobii_file) if tobii_file.endswith('.csv') else pd.read_excel(tobii_file)
            dialogflow_data = pd.read_csv(dialogflow_file) if dialogflow_file.endswith('.csv') else pd.read_excel(
                dialogflow_file)
            openface_data = pd.read_csv(openface_file) if openface_file.endswith('.csv') else pd.read_excel(
                openface_file)
            ...
            if 'etimestamp' in heartrate_data.columns:
                heartrate_times = pd.to_datetime(heartrate_data['etimestamp'], unit='ms',
                                                 errors='coerce').dt.tz_localize('UTC').dt.tz_convert('Asia/Tokyo')
            else:
                raise ValueError("Heart Rate file does not contain 'etimestamp' column")

            if 'etimestamp' in tobii_data.columns:
                tobii_times = pd.to_datetime(tobii_data['etimestamp'], unit='ms', errors='coerce').dt.tz_localize(
                    'UTC').dt.tz_convert('Asia/Tokyo')
            else:
                raise ValueError("Tobii file does not contain 'etimestamp' column")

            if 'etimestamp' in dialogflow_data.columns:
                dialogflow_times = pd.to_datetime(dialogflow_data['etimestamp'], unit='ms',
                                                  errors='coerce').dt.tz_localize('UTC').dt.tz_convert('Asia/Tokyo')
            else:
                raise ValueError("Dialogflow file does not contain 'etimestamp' column")

            if 'timestamp' in openface_data.columns:
                openface_times = pd.to_datetime(openface_data['timestamp'], unit='ms', errors='coerce')
                openface_times = openface_times.dt.tz_localize('UTC').dt.tz_convert('Asia/Tokyo')
            else:
            ...
            participant_name = heartrate_data['participant input'].iloc[
                0] if 'participant input' in heartrate_data.columns else 'Unknown'
