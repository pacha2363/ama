...
import os
import pandas as pd
from PyQt5.QtWidgets import QVBoxLayout, QLabel, QFileDialog, QFrame, QPushButton, QHBoxLayout, QMessageBox, QScrollArea, QWidget
...
class SystemChoiceFrame(QFrame):
    ...
    def display_graph(self):
        ...
        first_strategy_values = [self.first_strategy_stats['average'], self.first_strategy_stats['std_dev'], self.first_strategy_stats['min'], self.first_strategy_stats['max']]
        second_strategy_values = [self.second_strategy_stats['average'], self.second_strategy_stats['std_dev'], self.second_strategy_stats['min'], self.second_strategy_stats['max']]
    ...
    def display_preferred_system_graph(self):
        preferred_system_counts = self.system_choice_data['Prefered System'].value_counts()

        plt.figure(figsize=(6, 4))
        preferred_system_counts.plot(kind='bar', color='skyblue')
        plt.xlabel('Preferred System')
        plt.ylabel('Count')
        plt.title('Preferred System Counts')
        ...
        # Create a unique filename with timestamp
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        self.preferred_system_graph_filename = os.path.join(output_dir, f"preferred_system_data_{timestamp}.png")
