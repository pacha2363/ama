...
class HeartRateTest1Block(QFrame):
    def __init__(self, parent=None):
        ...
        self.upload_button_hr = QPushButton("Upload HR Test 1 File")
        self.upload_button_hr.clicked.connect(self.upload_hr_test1_file)
        self.button_layout.addWidget(self.upload_button_hr)

        self.layout.addLayout(self.button_layout)

        self.test_data = None

    def upload_hr_test1_file(self):
        options = QFileDialog.Options()
        file_path, _ = QFileDialog.getOpenFileName(self, "Select HR Test 1 File", "", "CSV Files (*.csv);;Excel Files (*.xlsx)", options=options)
        if file_path:
            self.process_hr_test1_data(file_path)

    def process_hr_test1_data(self, file_path):
        ...
        self.display_hr_test1_summary(self.test_data)

    def display_hr_test1_summary(self, participant_label):
